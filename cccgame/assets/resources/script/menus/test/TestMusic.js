...
cc.Class({
    ...
    onLoad () {
        ...
        // 初始化音乐列表
        BaseMusic.Instance().LoadMuiscList(()=>{
            BaseMusic.Instance().MusicPlay(BaseMusic_List.Music_1);
        });
    ...
    OnClick(touch) {
        ...
        if (touch.target.name == 'btn_close') {
            ...
            // 加载网络音乐，成功后播放
            BaseMusic.Instance().LoadNet('bg_1001.mp3',()=>{
                console.log("load music succ...");
                BaseMusic.Instance().MusicPlay('bg_1001.mp3');
            });
        }
        else if(touch.target.name == 'btn_test2') {
            BaseMusic.Instance().SoundPlay(BaseMusic_List.Sound_ReadyGo);
        }
